...
- เพิ่มระบบสลับแสดงผลสไลด์บาร์เพลย์ลิสต์ (Toggle Playlist Slide Bar):
  ...
  - เชื่อมโยงสถานะเพื่อควบคุมและจัดการพิกเซลการเรนเดอร์ของ `playlistRail` ในหน้าผู้ใช้งานจริง
- เพิ่มคำแนะนำคำค้นหาในการสืบค้นเพลย์ลิสต์ YouTube (Local Playlist Suggestions for YouTube Playlist Search):
  - ดึงรายการชื่อเพลย์ลิสต์ร้านที่มีอยู่แล้วในระบบมาสร้างเป็นปุ่มแนะนำ (Suggestions) คีย์เวิร์ดใต้ช่องค้นหา
  - พัฒนาระบบคลิกค้นหาอัจฉริยะ (Smart Click-to-Search) เมื่อกดปุ่มแนะนำ ระบบจะทำการป้อนข้อมูลลงในกล่องค้นหาพร้อมสั่งเรียก API ค้นหาไปยัง YouTube ทันทีโดยไม่ต้องพิมพ์
...
### Fixed
- แก้ไขแถบความคืบหน้า (Seek Bar) ไม่แสดงและไม่มีพื้นที่ให้กดในหน้าผู้ดูแลระบบ (/admin):
  - เอาคลาส `overflow-hidden` ออกจากคอนเทนเนอร์บาร์ควบคุมหลัก `player-ambient` เพื่อให้ Seek Bar ที่อยู่ขอบบนสุดสามารถแสดงผลได้สมบูรณ์แบบไม่โดนตัดทิ้งเมื่อแถบแนะนำเพลย์ลิสต์ (`playlistRail`) ถูกปิด/ไม่เรนเดอร์ในหน้า admin
- แก้ไขบั๊กแถบความคืบหน้า (Seek Bar) ไม่สามารถกดหรือลากเวลาเล่นเพลงได้ (Seek Bar Refinement & Pointer-Events Fix):
  - ขยายพื้นที่สัมผัสสำหรับการคลิกและลากแถบ Progress Bar ในหน้าจอควบคุมจาก `h-1` (4px) ขึ้นเป็น `h-5` (20px) ทำให้กดยืดหยุ่นง่ายดายทั้งบนอุปกรณ์เดสก์ท็อปและสมาร์ทโฟน
  - ลบตัวแสดงผล Custom Div ทับซ้อนที่บดบัง Pointer Events และเปลี่ยนมาใช้ Range และ Track ดั้งเดิมจาก Radix UI Slider โดยตรง ทำให้ค่าเวลาแสดงผลตรงตัว 100%
  - เพิ่มดีไซน์หรูหราแบบ Spotify: ซ่อนปุ่มกลม (Thumb) เป็น `opacity-0` และขยายความหนาของเส้นจาก `h-1` เป็น `h-1.5` อัตโนมัติเมื่อ Hover หรือ Focus-within
